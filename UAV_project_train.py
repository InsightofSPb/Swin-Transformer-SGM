...
f1_tr_sb, f1_tr_sf, f1_tr_ub, f1_tr_uf = [], [], [], []
...
for epoch in range(epochs):
	...
	for i, (sat_img, uav_img, target) in enumerate(dataloader):
		...
		trainLoss += loss.item()
		_, predicted = torch.max(sat_backgr_out.data, 1)
		f1_sb.extend(target.tolist())
		f1_sf.extend(predicted.tolist())		
	...
	f1_tr_sb.append(f1_score(f1_sb, f1_sf, average='macro'))
